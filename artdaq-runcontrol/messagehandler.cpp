...
void customLogWriter(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
  ...
  case QtInfoMsg:
    ...
    break;
  case QtWarningMsg:
    LOG_WRITE(std::cout, COLOR_WARN, "WARN", localMsg.constData(), file, context.line, function);
    break;
  case QtCriticalMsg:
    LOG_WRITE(std::cout, COLOR_CRITICAL, "CRIT", localMsg.constData(), file, context.line, function);
    break;
  case QtFatalMsg:
